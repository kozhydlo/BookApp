...
export default function BookPage() {
  const { id } = useParams();
  const router = useRouter(); // використаємо для навігації
  ...
  const handleGoBack = () => {
    router.push("/"); // повернення на головну
  };

  const handleSearch = () => {
    router.push("/search"); // перехід на сторінку пошуку
  };
    ...
    <motion.div transition={{ type: 'spring', damping: 40, mass: 0.75 }}
      ...
      <motion.section transition={{ type: 'spring', damping: 44, mass: 0.75 }}
        ...
        <div className={styles.icons}>
          ...
          <i style={iconStyle} className="fas fa-search" onClick={handleSearch} style={{ cursor: 'pointer' }}></i>
        </div>
      </motion.section>
